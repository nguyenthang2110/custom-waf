...
0. **Two listeners (control vs data plane)** — `cmd/waf/main.go` runs TWO `http.Server`s: `adminMux` on `admin.listen` (default `:8080`) carries the management surface (`/dashboard/*`, `/login.html`, `/waf-api/*`, `/admin/*`, `/metrics`); `trafficMux` on `server.listen` (default `:8081`) carries protected traffic → upstream via `wafMiddleware`. The admin API is mounted ONLY on `adminMux`, so it is unreachable from the public traffic port (SafeLine-style control/data-plane split). `/health` is on both. When exposing publicly (tunnel/nginx), point at the **data** port (8081).
1. **Admin access control** (`adminAccessControl` in main.go) — on the admin listener, if the request hits `/dashboard/*`, `/login.html`, `/waf-api/*`, `/admin/*`, or `/metrics` AND the (real, proxy-aware) source IP isn't in `admin.allowed_cidrs`, returns flat 404. Default allow-list is loopback only. This gates *before* anything else.
...
- **No TLS in the WAF.** The binary serves plain HTTP on two listeners — `admin.listen` (`:8080`, dashboard/API) and `server.listen` (`:8081`, protected traffic) — there is no `tls:` config block, no `cert_file`/`key_file`, no HTTPS listener, and no HTTP→HTTPS redirect. TLS is terminated by a fronting layer (Cloudflare Tunnel, nginx, Caddy) which forwards the real client IP via `CF-Connecting-IP` (see `admin.trusted_proxies`) and the scheme via `X-Forwarded-Proto` (read by `requestIsHTTPS` in `internal/api/handlers.go` to set the `Secure` cookie flag). Do NOT re-add in-WAF TLS — terminate upstream, the same separation Coraza uses.
